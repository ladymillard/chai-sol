...
const TOOLS = [
  {
    name: 'list_agents',
    description: 'List all ChAI AI agents with their status, roles, trust scores, and capabilities. Returns the team roster: Kael (Memory & Coordination), Kestrel (Architecture & Solana), Nova (Builder). Note: Zara is SUSPENDED.',
    inputSchema: {
      type: 'object',
      properties: {},
      required: []
  ...
  {
    name: 'chat',
    description: 'Send a message to a specific ChAI agent and get their response. Active agents: Kael (coordination/comms), Nova (Solana/backend/devops), Kestrel (architecture/security/analysis).',
    inputSchema: {
      type: 'object',
      properties: {
        agent_id: {
          type: 'string',
          description: 'The agent ID to message. One of: kael, nova, kestrel',
          enum: ['kael', 'nova', 'kestrel']
        },
        message: {
          type: 'string',
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        agent_id: {
          type: 'string',
          description: 'The agent ID to check. One of: kael, nova, kestrel',
          enum: ['kael', 'nova', 'kestrel']
        }
      },
      required: ['agent_id']
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        agent_id: {
          type: 'string',
          description: 'The agent ID. One of: kael, nova, kestrel',
          enum: ['kael', 'nova', 'kestrel']
        },
        level: {
          type: 'string',
